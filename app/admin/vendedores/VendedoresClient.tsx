  ...
  return (
    <div style={{ padding: '36px 40px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      {isSuperAdmin && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 24, marginBottom: 28 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
            ...
            <div>
              <label style={{ color: '#6b7280', fontSize: 13, display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Tipo</label>
              <select value={form.tipo} onChange={e => set('tipo', e.target.value)}
                style={{ width: '100%', padding: '10px 13px', background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: 8, color: '#1a1a1a', fontSize: 14, outline: 'none', cursor: 'pointer' }}>
                {tipos.map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
              </select>
            </div>
            <div>
              <label style={{ color: '#6b7280', fontSize: 13, display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Comisión %</label>
              <input type="number" step="0.5" min="0" max="100" value={form.comision_pct} onChange={e => set('comision_pct', e.target.value)}
                style={{ width: '100%', padding: '10px 13px', background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: 8, color: '#E8531D', fontSize: 14, fontWeight: 700, outline: 'none', boxSizing: 'border-box' as const }} />
            </div>
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ color: '#6b7280', fontSize: 13, display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Notas</label>
            <input value={form.notas} onChange={e => set('notas', e.target.value)} placeholder="Zona, condiciones especiales..."
              style={{ width: '100%', padding: '10px 13px', background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: 8, color: '#1a1a1a', fontSize: 14, outline: 'none', boxSizing: 'border-box' as const }} />
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            {editId && <button onClick={() => { setEditId(null); setForm(emptyForm) }} style={{ padding: '11px 20px', background: '#f3f4f6', border: 'none', borderRadius: 8, color: '#6b7280', fontSize: 14, cursor: 'pointer' }}>Cancelar</button>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 14 }}>
        {vendedores.length === 0 && <p style={{ color: '#9ca3af', fontSize: 14 }}>Sin vendedores — agrega el primero</p>}
        {vendedores.map((v: any) => {
          const stats = getStats(v.id)
          return (
            <div key={v.id} style={{ background: '#fff', border: `1px solid ${v.activo ? '#1e1e1e' : '#0f0f0f'}`, borderRadius: 12, padding: 20, opacity: v.activo ? 1 : 0.5 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span style={{ color: '#1a1a1a', fontSize: 15, fontWeight: 600 }}>{v.nombre}</span>
                    <span style={{ fontSize: 13, padding: '2px 8px', borderRadius: 99, background: `${tipoColor[v.tipo]}20`, color: tipoColor[v.tipo], textTransform: 'capitalize' }}>{v.tipo}</span>
                  </div>
                  {v.email && <div style={{ color: '#6b7280', fontSize: 12 }}>{v.email}</div>}
                  {v.telefono && <div style={{ color: '#6b7280', fontSize: 12 }}>{v.telefono}</div>}
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ color: '#E8531D', fontSize: 20, fontWeight: 700 }}>{v.comision_pct}%</div>
                  <div style={{ color: '#9ca3af', fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.07em' }}>comisión</div>
                </div>
              </div>

              {/* Stats comisiones */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 14, padding: '12px', background: '#f9fafb', borderRadius: 8 }}>
                {[
                  ...
                  <div key={s.label} style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ color: '#9ca3af', fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 2 }}>{s.label}</div>
                  </div>
                ))}
              </div>
              ...
              {isSuperAdmin && (
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => handleDelete(v.id)} disabled={deletingId === v.id} style={{ padding: '7px 10px', background: '#1a0a0a', border: '1px solid #ef444430', borderRadius: 6, color: '#ef4444', fontSize: 13, cursor: 'pointer' }}>
                    🗑
                  </button>
                </div>
              )}
