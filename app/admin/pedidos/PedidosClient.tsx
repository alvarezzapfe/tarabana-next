    ...
    <div style={{ padding: '36px 40px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: '#555', fontSize: 13 }}>{data.length} pedidos totales</p>
        </div>
        {canEdit && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 28 }}>
        {[
          ...
          <div key={s.label} style={{ background: '#111', border: '1px solid #1a1a1a', borderRadius: 10, padding: '16px 20px' }}>
            <p style={{ color: '#555', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 6px' }}>{s.label}</p>
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid #161616' }}>
              {['#', 'Cliente', 'Productos', 'Total', 'Entrega', 'Pago', canEdit ? 'Acciones' : ''].map((h, i) => (
                <th key={i} style={{ color: '#3a3a3a', fontSize: 10.5, textAlign: 'left', padding: '10px 16px', textTransform: 'uppercase', letterSpacing: '0.07em', whiteSpace: 'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.length ? data.map(p => {
              const sc = statusConfig[p.status] || { label: p.status, color: '#555', bg: '#55555518' }
              ...
              return (
                <tr key={p.id} style={{ borderBottom: '1px solid #141414' }}>
                  <td style={{ padding: '12px 16px' }}>
                    <p style={{ margin: 0, color: '#444', fontSize: 11, fontFamily: 'monospace' }}>#{p.id.slice(-6).toUpperCase()}</p>
                    <p style={{ margin: '2px 0 0', color: '#2a2a2a', fontSize: 10.5 }}>{fecha.toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}</p>
                    <p style={{ margin: '1px 0 0', color: '#2a2a2a', fontSize: 10 }}>{fecha.toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })}</p>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <p style={{ margin: 0, color: '#ddd', fontSize: 13 }}>{cliente?.full_name || '—'}</p>
                    <p style={{ margin: '1px 0 0', color: '#444', fontSize: 11 }}>{cliente?.email}</p>
                    <span style={{ background: p.tipo_precio === 'taproom' ? '#f59e0b18' : '#1a1a1a', color: p.tipo_precio === 'taproom' ? '#f59e0b' : '#555', fontSize: 10, padding: '1px 6px', borderRadius: 99, marginTop: 3, display: 'inline-block' }}>
                      {p.tipo_precio === 'taproom' ? 'Taproom' : 'Público'}
                    </span>
                  </td>
                  <td style={{ padding: '12px 16px', maxWidth: 200 }}>
                    {items?.map((item: any, i: number) => (
                      <p key={i} style={{ margin: '1px 0', color: '#666', fontSize: 11.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {item.cantidad}× {item.productos?.nombre}
                      </p>
                    ))}
                  </td>
                  <td style={{ padding: '12px 16px', color: '#E8531D', fontSize: 15, fontWeight: 700, whiteSpace: 'nowrap' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        <select value={p.status} onChange={e => updateStatus(p.id, e.target.value)}
                          style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 6, color: '#888', fontSize: 11.5, padding: '5px 8px', cursor: 'pointer', width: '100%' }}>
              ...
              <tr><td colSpan={7} style={{ color: '#444', textAlign: 'center', padding: '60px 20px', fontSize: 14 }}>No hay pedidos aún</td></tr>
            )}
          </tbody>
        </table>
      </div>
