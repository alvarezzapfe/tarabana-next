    ...
    <div style={{ padding: '36px 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: '#6b7280', fontSize: 13 }}>{total} registrados · {taps} con tap/bar</p>
        </div>
        {canEdit && (
              ...
              <tr key={c.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                <td style={{ padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#E8531D', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
                      {(c.full_name || c.email || '?')[0].toUpperCase()}
                    </div>
                    <div>
                      <p style={{ color: '#1a1a1a', margin: 0, fontSize: 13, fontWeight: 500 }}>{c.full_name || '—'}</p>
                      <p style={{ color: '#9ca3af', margin: '1px 0 0', fontSize: 11 }}>{c.email}</p>
                    </div>
                  </div>
                </td>
                <td style={{ padding: '12px 14px', color: '#6b7280', fontSize: 14, whiteSpace: 'nowrap' }}>{c.phone || '—'}</td>
                <td style={{ padding: '12px 14px', color: '#6b7280', fontSize: 14 }}>{tipoLabel[c.tipo_consumidor] || '—'}</td>
                <td style={{ padding: '12px 14px', maxWidth: 140 }}>
                  <p style={{ color: '#6b7280', margin: 0, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.direccion_entrega || '—'}</p>
                  {c.ciudad && <p style={{ color: '#9ca3af', margin: '1px 0 0', fontSize: 11 }}>{c.ciudad}</p>}
                </td>
                <td style={{ padding: '12px 14px', color: '#6b7280', fontSize: 14, maxWidth: 140 }}>
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <a href={`/admin/pedidos/nuevo?cliente=${c.id}`} style={{
                        ...
                        background: '#f3f4f6', color: '#6b7280', borderRadius: 6, textDecoration: 'none', fontSize: 13, fontWeight: 500, whiteSpace: 'nowrap'
                      }}>
                      ...
                      <a href={`/admin/clientes/edit/${c.id}`} style={{
                        display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px',
                        background: '#f3f4f6', color: '#6b7280', borderRadius: 6, textDecoration: 'none', fontSize: 13, fontWeight: 500
                      }}>
